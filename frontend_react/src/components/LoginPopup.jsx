...
const LoginPopup = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        <h2 className="text-2xl font-bold text-white mb-8 text-center">{locale('login.welcome')}</h2>
        ...
        <button onClick={onClose} className="mt-8 w-full text-gray-500 hover:text-gray-300 text-sm transition-colors">{locale('common.close')}</button>
      </motion.div>
    </div>
  );
};
